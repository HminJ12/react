...
function App() {
  ...
  return (
    <AppContext.Provider value={{
      _pointArr, _setPointArr, fnChangePoint, _setReadOnly, _readOnly, _setShowBtn, _reset, _setStyle, _style, _listCnt
    }}>
      <h1>Context</h1>
      <hr />
      <CompUl/>
      <hr />
      <CompResult/>
      {(_showBtn)&&<button onClick={e=>{_setReset(p => !p);_setReadOnly(false);_setPointArr(new Array(3).fill('x')); _setStyle('#FFF');_setListCnt(Math.ceil(Math.random()*3))}}>다시시작하기</button>}
    </AppContext.Provider>
  );
}
